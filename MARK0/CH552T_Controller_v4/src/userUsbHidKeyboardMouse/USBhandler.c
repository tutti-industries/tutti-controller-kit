...
// USER_USB_RAM はツールメニューの USB Settings で決まる。
// この#errorが出たら「USER CODE w/ 148B USB ram」を選ぶこと(HIDに138B必要)。
        ...
        USB_REQ_TYP_STANDARD) // Not standard request
    {
      ...
      case USB_REQ_TYP_CLASS: {
        ...
        case 0x0B: // SET_PROTOCOL
          len = 0;  // ACK without data
          break;
        case 0x03: // GET_PROTOCOL
          Ep0Buffer[0] = 1; // Report Protocol mode
          len = 1;
          break;
        default:
          len = 0xFF; // command not supported
